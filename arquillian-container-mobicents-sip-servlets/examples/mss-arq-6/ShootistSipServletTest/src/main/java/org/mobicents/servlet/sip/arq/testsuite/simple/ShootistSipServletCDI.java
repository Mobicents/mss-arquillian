...
public class ShootistSipServletCDI
{
	...
	private static transient Logger logger = Logger.getLogger(ShootistSipServletCDI.class);
...
//	}		
	...
	protected void doProvisionalResponse(@Observes @ProvisionalResponse SipServletResponse resp)
		...
		try {
			...
			if(servletContext.getInitParameter("cancelOn1xx") != null) {
				try {
					Thread.sleep(500);
			...
			if(resp.getStatus() == 180 && servletContext.getInitParameter("cancel") != null) {					
				String timeToWaitString = servletContext.getInitParameter("servletTimer");
	...
	protected void doSuccessResponse(@Observes @SuccessResponse SipServletResponse sipServletResponse)
			throws ServletException, IOException {
		try {
			access();
			...
			if (status == SipServletResponse.SC_OK && "INVITE".equalsIgnoreCase(sipServletResponse.getMethod())) {
				...
				if(sipServletResponse.getRequest().isInitial() && !(sipServletResponse.getFrom().getURI() instanceof TelURL) && !(sipServletResponse.getTo().getURI() instanceof TelURL) &&
					...
					if(sipServletResponse.getApplicationSession().getAttribute(ENCODE_URI) == null && servletContext.getInitParameter(NO_BYE) == null) {
						String timeToWaitForBye = servletContext.getInitParameter("timeToWaitForBye");
						int delay = 2000;
			...
			if(servletContext.getInitParameter("closeReliableChannel")!= null) {
				timerService.createTimer(sipFactory.createApplicationSession(), Long.valueOf(servletContext.getInitParameter("timeout")), false, "" + sipServletResponse.getInitialRemotePort());
			}
			if(servletContext.getInitParameter("testKeepAlive") != null) {
				SipConnector[] sipConnectors = (SipConnector[]) servletContext.getAttribute("org.mobicents.servlet.sip.SIP_CONNECTORS");
				for (SipConnector sipConnector : sipConnectors) {
					if(sipConnector.getIpAddress().equals(sipServletResponse.getLocalAddr()) && sipConnector.getPort() == sipServletResponse.getLocalPort() && sipConnector.getTransport().equals(sipServletResponse.getTransport())) {
						...
						sipServletResponse.getApplicationSession().setAttribute("keepAlivetoSend", Integer.valueOf(servletContext.getInitParameter("testKeepAlive")));
...
//	}
	...
	protected void doResponse(@Observes @Response SipServletResponse resp) throws ServletException,
			IOException {
		ClassLoader cl = Thread.currentThread().getContextClassLoader();
		if(!cl.getClass().getSimpleName().equals("WebappClassLoader")) {
			logger.error("ClassLoader " + cl);
...
//		super.doResponse(resp);
	...
	protected void doErrorResponse(@Observes @ErrorResponse SipServletResponse sipServletResponse)
			throws ServletException, IOException {
		try {
			access();
			logger.info("Got : " + sipServletResponse.getStatus());
			if(	servletContext.getInitParameter("cancel") != null) {
	...
	protected void doBye(@Observes @Bye SipServletRequest req) throws ServletException,
			IOException {
		try {
			access();
	 ...
	 */
	public void servletInitialized(@Observes @ServletInitialized SipServletContextEvent ce) {
		try {
			access();
			ClassLoader cl = Thread.currentThread().getContextClassLoader();		
//			if(!cl.getClass().getSimpleName().equals("WebappClassLoader")) {
...
//				throw new IllegalArgumentException("Bad Context Classloader : " + cl);
//			}
			SipFactory sipFactory = (SipFactory)ce.getServletContext().getAttribute(javax.servlet.sip.SipServlet.SIP_FACTORY);
			...
			if(ce.getServletContext().getInitParameter("urlType") != null) {
				if(ce.getServletContext().getInitParameter("urlType").equalsIgnoreCase("tel")) {
					...
					if(ce.getServletContext().getInitParameter("enum") != null) {
						DNSResolver dnsResolver = (DNSResolver) servletContext.getAttribute("org.mobicents.servlet.sip.DNS_RESOLVER");
			...
			if(outboundInterface != null) {
				List<SipURI> outboundInterfaces = (List<SipURI>)servletContext.getAttribute(javax.servlet.sip.SipServlet.OUTBOUND_INTERFACES);
			...
			if(ce.getServletContext().getInitParameter("cancel") != null) {
				...
				if(timeToWaitString == null) {
					...
					if(servletContext.getInitParameter("servletTimer") != null) {
